...
{
public:
    vector<int> postorderTraversal(TreeNode *root) // Iteration 1 stack, just reverse the vector instead of 2 stacks
    {
        // TC: O(2*n), SC: O(n)
       if(root==NULL)
       {
    ...
    vector<int> postorderTraversal2(TreeNode *root) // Using Iteration 2 stack
    {
        // TC: O(n), SC: O(2*n)
        vector<int> ans;
        if (!root)
