...
public:
    int solve(vector<int>& slices, int picksLeft, int index, int endIndex, vector<vector<int>>& memo) {
        ...
        if(memo[index][picksLeft-1]!=-1)
        {
            return memo[index][picksLeft-1];
        }
        ...
        return memo[index][picksLeft-1]=max(take, skip);
    }

    int maxSizeSlices(vector<int>& slices) {
        int picks = slices.size() / 3;
        vector<vector<int>> memo1(slices.size(), vector<int>(picks, -1));
        vector<vector<int>> memo2(slices.size(), vector<int>(picks, -1));
